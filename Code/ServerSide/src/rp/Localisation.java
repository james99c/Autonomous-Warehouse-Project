...
public class Localisation {
	...
	public ArrayList<GridPoint> getMatchingGridPoints(ArrayList<Integer> _surroundingDistances, ArrayList<GridPoint> possibleGridPoints){
	...
	public Integer[] getDistancesAtGridPoint(GridPoint point) {
		...
		// East
		while (robotX + output[0] + 1 < map.getWidth()
					&& arrayMap[robotX + output[0] + 1][robotY].isAvailable(-1.0f)) {
			++output[0];
		}
		// West
		while (robotX - output[1] - 1 >= 0
					&& arrayMap[robotX - output[1] - 1][robotY].isAvailable(-1.0f)) {
			++output[1];
		}
		// South
		while (robotY - output[2] - 1 >= 0
					&& arrayMap[robotX][robotY - output[2]].isAvailable(-1.0f)) {
			++output[2];
		}
		// North
		while (robotY + output[3] + 1 < map.getHeight()
					&& arrayMap[robotX][robotY + output[3] + 1].isAvailable(-1.0f)) {
			++output[3];
		}
